...
class _TimePickerFragmentContext {
  ...
  final ValueChanged<dynamic> onTimeChange;
...
class TimePickerHeader extends StatefulWidget {
  TimePickerHeader({
    required this.selectedTime,
    required this.mode,
  ...
  @override
  State<TimePickerHeader> createState() => _TimePickerHeaderState();
}

class _TimePickerHeaderState extends State<TimePickerHeader> {
  ...
  Widget build(BuildContext context) {
    ...
    _TimePickerFragmentContext(
      ...
      mode: widget.mode,
      onTimeChange: widget.onChanged,
      onModeChange: _handleChangeMode,
      onHourDoubleTapped: widget.onHourDoubleTapped,
      onMinuteDoubleTapped: widget.onMinuteDoubleTapped,
      use24HourDials: widget.use24HourDials,
    );
...
class _HourControl extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    dynamic hoursFromSelected(int hoursToAdd) {
    ...
    final dynamic nextHour = hoursFromSelected(1);
    final String formattedNextHour = localizations.formatHour(
      nextHour,
      alwaysUse24HourFormat: alwaysUse24HourFormat,
    );
    final dynamic previousHour = hoursFromSelected(-1);
...
class _DayPeriodControl extends StatelessWidget {
  ...
  void _togglePeriod() {
    ...
    final dynamic newTime = selectedTime.replacing(hour: newHour);
    onChanged(newTime);
  }
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  dynamic _notifyOnChangedIfNeeded({bool roundMinutes = false}) {
  ...
  void _handleTapUp(TapUpDetails details) {
    ...
    final dynamic newTime = _notifyOnChangedIfNeeded(roundMinutes: true);
    if (widget.mode == _TimePickerMode.hour) {
      if (widget.use24HourDials) {
  ...
  static const List<dynamic> _amHours = <dynamic>[
  ...
  static const List<dynamic> _twentyFourHours = <dynamic>[
  ...
  List<_TappableLabel> _build12HourRing(TextTheme textTheme, Color color) =>
      <_TappableLabel>[
        for (final dynamic timeOfDay in _amHours)
          _buildTappableLabel(
            textTheme,
            color,
            timeOfDay.hour,
  ...
  List<_TappableLabel> _buildMinutes(TextTheme textTheme, Color color) {
    const List<dynamic> _minuteMarkerValues = <dynamic>[
      TimeOfDay(hour: 0, minute: 0),
    ...
    return <_TappableLabel>[
      for (final dynamic timeOfDay in _minuteMarkerValues)
        _buildTappableLabel(
          textTheme,
          color,
          timeOfDay.minute,
...
class _TimePickerInput extends StatefulWidget {
  ...
  final dynamic initialSelectedTime;
...
class _TimePickerInputState extends State<_TimePickerInput>
  ...
  void _handleDayPeriodChanged(dynamic value) {
...
class _TimePickerDialogState extends State<TimePickerDialog>
  ...
  void _handleTimeChanged(dynamic value) {
    _vibrate();
    setState(() {
      _selectedTime.value = value;
    });
  ...
  void _handleOk() {
    ...
    var data = {"day": dayList, "time": selectedTime.value};
...
Future<dynamic> showDayTimePicker({
